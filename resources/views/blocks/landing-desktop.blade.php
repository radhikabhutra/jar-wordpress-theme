  ...
  <nav class="flex justify-end items-center gap-6">
    ...
    <div class="relative group">
      <button
        ...
        <svg
          class="w-4 h-4 transform transition-transform duration-200 group-hover:rotate-180 group-hover:text-[#7029CC]"
...
<main class="bg-white min-h-screen flex flex-col items-center w-full">
      ...
      @while (have_rows('field_awards_section'))
        ...
        <div class="flex flex-col justify-center items-center gap-3">
          ...
          @if (get_sub_field('field_award_image'))
            <img src="{{ get_sub_field('field_award_image') }}" alt="{{ get_sub_field('field_award_title') }}"
              class="w-20 h-auto" />
          @endif
          ...
          <div class="flex">
            ...
            @if (get_field('field_awards_left_vector'))
              <img src="{{ get_field('field_awards_left_vector') }}" alt="Left Vector" class="w-8 h-auto" />
            @endif
            ...
            @if (get_field('field_awards_right_vector'))
              <img src="{{ get_field('field_awards_right_vector') }}" alt="Right Vector" class="w-8 h-auto" />
            @endif
          </div>
        </div>
      @endwhile
      ...
      <div id="mentions-splide" class="splide w-full max-w-4xl mx-auto">
        <div class="splide__track">
          <ul class="splide__list">
            @while (have_rows('field_mentions_section'))
              @php(the_row())
              <li class="splide__slide flex flex-col items-center w-full">
                <div class="w-full flex justify-center">
                  <iframe src="{{ get_sub_field('field_mentions_video_url') }}" class="w-full max-w-4xl aspect-video"
                    allowfullscreen frameborder="0"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                    title="Mention Video"></iframe>
                </div>
  ...
  <section class="w-full bg-[#F4F4F5] py-20 flex flex-col items-center">
    ...
    @if (have_rows('field_why_save_in_jar_features'))
      <div class="grid grid-cols-3 justify-center gap-6 w-full max-w-6xl">
        @while (have_rows('field_why_save_in_jar_features'))
          @php(the_row())
          <div class="bg-white rounded-[20px] flex flex-col items-center p-5 border border-[#E9E4F0]">
            <img src="{{ get_sub_field('field_why_save_in_jar_icon') }}"
              alt="{{ get_sub_field('field_why_save_in_jar_card_title') }}" class="h-[160px] w-auto mb-4" />
            <div class="text-xl font-bold text-[#1A003D] mb-2 text-center">
              {{ get_sub_field('field_why_save_in_jar_card_title') }}</div>
  ...
  <section id="how-jar-works-section" class="py-20 w-full flex flex-col justify-center items-center gap-12 mx-auto">
    ...
    <div id="lottie-container" class="w-full max-w-5xl mx-auto flex justify-center gap-6">
      @if (have_rows('field_how_jar_works_steps'))
        @while (have_rows('field_how_jar_works_steps'))
          @php(the_row())
          <figure class="w-[300px] h-[300px]" x-data="{ animation: '' }" x-init="lottie.loadAnimation({
              container: $el,
              renderer: 'svg',
              loop: true,
              autoplay: true,
    ...
    <div class="steps-container w-full max-w-5xl flex justify-center gap-6">
      @if (have_rows('field_how_jar_works_steps'))
        @while (have_rows('field_how_jar_works_steps'))
          @php(the_row())
          <div class="step flex flex-col justify-center items-center gap-2 w-[300px]"
            id="step-{{ get_sub_field('field_how_jar_works_step_number') }}">
            <div
              class="step-number w-8 h-8 bg-[#887A99] rounded-full flex items-center justify-center text-white font-bold">
    ...
    <div class="controls w-full flex justify-center gap-3">
      <button
        class="prev-step bg-[#43197A] text-white rounded-full w-10 h-10 flex justify-center items-center cursor-pointer">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
      </button>
      <button
        class="next-step bg-[#43197A] text-white rounded-full w-10 h-10 flex justify-center items-center cursor-pointer">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  <!-- Reviews Section -->
  <section x-data="reviewsComponent()" x-init="init()" x-intersect:enter="startAuto()"
    x-intersect:leave="stopAuto()" class="w-full px-[120px] py-20 bg-[#160829]">
        ...
        <div class="flex flex-col gap-2 items-center w-full">
          ...
          <div class="flex justify-center gap-4">
            <template x-for="(review, idx) in reviews.slice(4, 9)" :key="idx + 4">
              <img :src="review.image" :alt="review.name"
                ...
                :class="(idx + 4) === active ? 'border-4 border-[#FFF] shadow-lg scale-110 z-10' :
                    'border-[#E9E4F0] opacity-20 z-0'"
          ...
          <div class="flex justify-center gap-9">
            <template x-for="(review, idx) in reviews.slice(9, 13)" :key="idx + 9">
              <img :src="review.image" :alt="review.name"
                class="w-[60px] h-[60px] rounded-full object-cover border transition-all duration-300 cursor-pointer"
                :class="(idx + 9) === active ? 'border-4 border-[#FFF] shadow-lg scale-110 z-10' :
                    'border-[#E9E4F0] opacity-20 z-0'"
